---
title: "DAACS_UMGC1_Testing for Multivariate Outliers_OR"
author: "Oxana Rosca"
date: "2022-12-23"
output:
...
Subset the UMGC1 total scores for four assessments
```{r}
global_umgc1<-global_ua2umgc1[which(global_ua2umgc1$college=='UMGC1'),]
totalScores <- global_umgc1[,c(21,49,236,39)]
```
...
pairs.panels(outliers_totalScores,bg=c("grey","blue")[(mah.distance>18.47)+1],
             pch=21,main="Multivariate Outliers for UMGC1 Total Scores")
```
Add the ID variable to the dataset:
```{r}
outliers_totalScores$srlTime <- global_umgc1$srlTime
outliers_totalScores$mathTime <- global_umgc1$mathTime
outliers_totalScores$readTime <- global_umgc1$readTime
outliers_totalScores$writeTime <- global_umgc1$writeTime
outliers_totalScores$DAACS_ID <- global_umgc1$DAACS_ID
```
...
*Save New File*
```{r}
saveRDS(outliers_totalScores, file = 'MV outliers UMGC1_totalScores.rds')
write_sav(outliers_totalScores, "MV outliers UMGC1_totalScores.sav")
write.table(outliers_totalScores, 'MV outliers UMGC1_totalScores.csv', sep=',')
```
...
pairs.panels(outliers_df,bg=c("grey","blue")[(mah.distance>24.32)+1],
             gap=0,pch=21,main="Multivariate Outliers for UMGC1 Math Domains")
```
Add the ID variable to the dataset:
```{r}
outliers_df$mathTime <- global_umgc1$mathTime
outliers_df$DAACS_ID <- global_umgc1$DAACS_ID
```
...
*Save New File*
```{r}
saveRDS(outliers_math, file = 'multivariate outliers UMGC1_math.rds')
write_sav(outliers_math, "multivariate outliers UMGC1_math.sav")
write.table(outliers_math, 'multivariate outliers UMGC1_math.csv', sep=',')
```
...
pairs.panels(outliers_df,bg=c("grey","blue")[(mah.distance>22.46)+1],
             gap=0,pch=21,main="Multivariate Outliers on UMGC1 Reading Domains")
```
Add the ID variable to the dataset:
```{r}
outliers_df$readTime <- global_umgc1$readTime
outliers_df$DAACS_ID <- global_umgc1$DAACS_ID
```
...
*Save New File*
```{r}
saveRDS(outliers_read, file = 'multivariate outliers UMGC1_read.rds')
write_sav(outliers_read, "multivariate outliers UMGC1_read.sav")
write.table(outliers_read, 'multivariate outliers UMGC1_read.csv', sep=',')
```
...
Add the ID variable to the dataset:
```{r}
outliers_df$writeTime <- global_umgc1$writeTime
outliers_df$DAACS_ID <- global_umgc1$DAACS_ID
```
...
*Save New File*
```{r}
saveRDS(outliers_write, file = 'multivariate outliers UMGC1_write.rds')
write_sav(outliers_write, "multivariate outliers_write UMGC1.sav")
write.table(outliers_write, 'multivariate outliers_write UMGC1.csv', sep=',')
```