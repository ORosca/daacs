---
title: "DAACS_UA2_Testing for Multivariate Outliers_OR"
author: "Oxana Rosca"
date: "2022-12-23"
output:
...
Subset the UA2 total scores for four assessments
```{r}
global_ua2<-global_ua2umgc1[which(global_ua2umgc1$college=='UA2'),]
totalScores <- global_ua2[,c(21,49,236,39)]
```
...
pairs.panels(outliers_totalScores,bg=c("grey","blue")[(mah.distance>18.47)+1],
             pch=21,main="Multivariate Outliers for UA2 Total Scores")
```
Add the ID variable to the dataset:
```{r}
outliers_totalScores$srlTime <- global_ua2$srlTime
outliers_totalScores$mathTime <- global_ua2$mathTime
outliers_totalScores$readTime <- global_ua2$readTime
outliers_totalScores$writeTime <- global_ua2$writeTime
outliers_totalScores$DAACS_ID <- global_ua2$DAACS_ID
```
...
*Save New File*
```{r}
saveRDS(outliers_totalScores, file = 'MV outliers UA2_totalScores.rds')
write_sav(outliers_totalScores, "MV outliers UA2_totalScores.sav")
write.table(outliers_totalScores, 'MV outliers UA2_totalScores.csv', sep=',')
```
...
pairs.panels(outliers_df,bg=c("grey","blue")[(mah.distance>24.32)+1],
             gap=0,pch=21,main="Multivariate Outliers for UA2 Math Domains")
```
Add the ID variable to the dataset:
```{r}
outliers_df$mathTime <- global_ua2$mathTime
outliers_df$DAACS_ID <- global_ua2$DAACS_ID
```
...
*Save New File*
```{r}
saveRDS(outliers_math, file = 'multivariate outliers UA2_math.rds')
write_sav(outliers_math, "multivariate outliers UA2_math.sav")
write.table(outliers_math, 'multivariate outliers UA2_math.csv', sep=',')
```
...
pairs.panels(outliers_df,bg=c("grey","blue")[(mah.distance>22.46)+1],
             gap=0,pch=21,main="Multivariate Outliers on UA2 Reading Domains")
```
Add the ID variable to the dataset:
```{r}
outliers_df$readTime <- global_ua2$readTime
outliers_df$DAACS_ID <- global_ua2$DAACS_ID
```
...
*Save New File*
```{r}
saveRDS(outliers_read, file = 'multivariate outliers UA2_read.rds')
write_sav(outliers_read, "multivariate outliers UA2_read.sav")
write.table(outliers_read, 'multivariate outliers UA2_read.csv', sep=',')
```
...
Add the ID variable to the dataset:
```{r}
outliers_df$writeTime <- global_ua2$writeTime
outliers_df$DAACS_ID <- global_ua2$DAACS_ID
```
...
*Save New File*
```{r}
saveRDS(outliers_write, file = 'multivariate outliers UA2_write.rds')
write_sav(outliers_write, "multivariate outliers_write UA2.sav")
write.table(outliers_write, 'multivariate outliers_write UA2.csv', sep=',')
```